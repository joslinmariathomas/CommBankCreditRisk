...
def plot_target_by_feature(df: pd.DataFrame, contract_col: str):
    ...
    sns.countplot(data=df, x=target_col, hue=contract_col, order=ordered_targets, ax=ax)
    ...
    df.groupby([target_col, contract_col]).size().unstack(fill_value=0).reindex(
        ordered_targets
    )
